...
- `checks` — anonymous eligibility check records (`profile` + `result` jsonb, no user_id). Anyone may insert; anyone holding the uuid may read (that uuid is the shareable `/result/[id]` link).

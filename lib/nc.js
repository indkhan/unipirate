...
// Merge curated static rules into the DB set as a fallback: the DB wins where it
// has a (country, qualification_type) row; static fills any gaps — e.g. new
// universal A-Level/IB/Studienkolleg rules that haven't been re-seeded yet. Keeps
// a stale database from showing UNCLEAR for combinations we actually cover.
export function mergeRecognitionRules(dbRules = [], staticRules = []) {
  if (!dbRules.length) return staticRules;
  const key = (r) => `${r.country}::${r.qualification_type}`;
  const seen = new Set(dbRules.map(key));
  return [...dbRules, ...staticRules.filter((r) => !seen.has(key(r)))];
}

// --- Recognition rule matching (operates on snake_case DB/seed rows) ---
// Priority: exact (country + qualification) -> universal qualification
// (country 'Any', e.g. A-Levels/IB regardless of country) -> country wildcard
// (qualification 'Any') -> 'Other / Not listed' fallback.
export function matchRecognitionRule(profile, rules = []) {
  if (!profile?.country || !profile?.qualification) return null;
  return (
    rules.find(
      (r) => r.country === profile.country && r.qualification_type === profile.qualification
    ) ||
    rules.find(
      (r) => r.country === 'Any' && r.qualification_type === profile.qualification
    ) ||
    rules.find((r) => r.country === profile.country && r.qualification_type === 'Any') ||
    rules.find((r) => r.country === 'Other / Not listed') ||
    null
  );
}
...
export function resolveRecognition(rule, profile) {
  ...
  // ib_subjects: general access only with HL Maths AND a HL natural science;
  // otherwise the subject-restricted (below) variant.
  if (rule.grade_threshold_kind === 'ib_subjects') {
    const hlMath = !!(profile?.ib_hl_math ?? profile?.ibHlMath);
    const hlScience = !!(profile?.ib_hl_science ?? profile?.ibHlScience);
    return normalizeRule(rule, !(hlMath && hlScience));
  }

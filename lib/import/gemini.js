...
// Wording-only coherence pass over checklist step labels. Dates never leave
// this function's caller: only labels go to the model, due_dates are
// re-attached untouched, and any digit drift discards the whole rewrite.
export async function rephraseChecklist(steps) {
  if (!geminiEnabled() || !steps?.length) return steps;

  const labels = steps.map((s) => s.label);
  const prompt = `Rewrite these university-application checklist steps so they read naturally and consistently (imperative voice, no jargon). Keep the meaning of each step EXACTLY the same. Do not add, remove, merge, or reorder steps. Do not add or remove any numbers.

Return a JSON array of exactly ${labels.length} strings, same order.

STEPS:
${JSON.stringify(labels)}`;

  const out = await callGemini(prompt, { type: 'array', items: { type: 'string' } });
  if (!Array.isArray(out) || out.length !== labels.length) return steps;

  const digits = (s) => (s.match(/\d+/g) || []).join(',');
  for (let i = 0; i < labels.length; i++) {
    if (typeof out[i] !== 'string' || !out[i].trim()) return steps;
    if (digits(out[i]) !== digits(labels[i])) return steps; // number drift → discard all
  }

  return steps.map((s, i) => ({ ...s, label: out[i].trim() }));
}
